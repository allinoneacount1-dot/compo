    ...
    <div className="p-6 max-w-[1200px]">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-sm font-semibold">Leaderboard</span>
        </div>
        <div className="flex items-center gap-2">
          {["All Time", "This Week", "Verified"].map((t) => (
            <button key={t} className={["px-3 py-1 text-[11px] rounded-lg transition-colors", t === "All Time" ? "bg-[#00ff9f]/10 text-[#00ff9f]" : "bg-[#222] text-[#52525b] hover:text-white"].join(" ")}>{t}</button>
          ))}
        </div>
      </div>
              ...
              <tr key={l.rank} className={["hover:bg-[#1a1a1a]", l.rank <= 3 ? "border-l-2 border-l-[#00ff9f]" : ""].join(" ")}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    {l.verified && <span className="text-[9px] bg-[#00ff9f]/10 text-[#00ff9f] px-1.5 py-0.5 rounded">VERIFIED</span>}
                  </div>
                  <span className="text-[10px] text-[#52525b]">{l.label}</span>
                </td>
                <td className="px-4 py-3 text-center">
                  <span className="text-[#00ff9f] font-bold text-[12px]">{l.alpha}</span>
                </td>
                <td className="px-4 py-3 text-right text-[#00ff9f] text-[11px]">{l.pnl24h}</td>
                <td className="px-4 py-3 text-right text-[#00ff9f] text-[11px]">{l.pnl7d}</td>
                <td className="px-4 py-3 text-center text-[#a1a1aa] text-[11px]">{l.winRate}%</td>
                <td className="px-4 py-3 text-center text-[#52525b] text-[11px]">{l.trades}</td>
              </tr>
            ))}
          </tbody>
        </table>
